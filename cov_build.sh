#!/usr/bin/env bash
#
# cov_build.sh - Coverity-friendly reproduction of the libnm_proxy_l1_tests
# GitHub Actions workflow (.github/workflows/libnm_proxy_L1_test.yml).
#
# Builds the Thunder ecosystem (ThunderTools, Thunder, ThunderInterfaces),
# then networkmanager with the Gnome libnm proxy so that Coverity can capture
# the compilation. Test execution, coverage and report upload steps from the
# workflow are intentionally omitted - Coverity only needs the build.
#
# Usage:
#   ./cov_build.sh
#
...
# By default we use the parent of the networkmanager checkout so the layout
# matches the workflow (Thunder/, ThunderTools/, install/, etc. live beside it).
