    ...
    namespace Plugin
    {
        ...
        class NetworkManager : public PluginHost::IPlugin, public PluginHost::JSONRPC, public PluginHost::ISubSystem::IInternet
        {
            ...
            public:
                ...
                void onActiveInterfaceChange(string prevActiveInterface, const string currentActiveinterface) override
                {
                    _parent.onActiveInterfaceChange(std::move(prevActiveInterface), currentActiveinterface);
                }

                void onIPAddressChange(string interface, const string ipversion, const string ipaddress, const Exchange::INetworkManager::IPStatus status) override
                {
                    _parent.onIPAddressChange(std::move(interface), ipversion, ipaddress, status);
                }

                void onRouteChange(string interface, string ipversion, const string ipaddress, string gateway, string primarydns) override
                {
                    _parent.onRouteChange(std::move(interface), std::move(ipversion), ipaddress, std::move(gateway), std::move(primarydns));
                }

                void onInternetStatusChange(const Exchange::INetworkManager::InternetStatus prevState, const Exchange::INetworkManager::InternetStatus currState, string interface, const string reason) override
                {
                    _parent.onInternetStatusChange(prevState, currState, std::move(interface), reason);
                }
        ...
        private:
            ...
            void onInterfaceStateChange(const Exchange::INetworkManager::InterfaceState state, string interface);
