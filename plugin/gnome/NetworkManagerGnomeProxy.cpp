    ...
    namespace Plugin
    {
        ...
        uint32_t NetworkManagerImplementation::GetIPSettings(string& interface /* @inout */, const string &ipVersion /* @in */, IPAddress& result /* @out */)
        {
            ...
            if(ipVersionInput.empty())
            {
                ...
                {
                    ipVersionInput = "IPV4";
		    NMLOG_INFO(" ------- GURU: 1. ipVersionInput = %s ------ ", ipVersionInput.c_str());
                }
                else if ((ip4Byte->len) && (ip6Array->len == 0))
                {
                    ipVersionInput = "IPV4";
		    NMLOG_INFO(" ------- GURU: 2. ipVersionInput = %s ------ ", ipVersionInput.c_str());
                }
                else if ((ip4Byte->len == 0) && (ip6Array->len))
                {
                    ipVersionInput = "IPV6";
		    NMLOG_INFO(" ------- GURU: 3. ipVersionInput = %s ------ ", ipVersionInput.c_str());
                }
            }
